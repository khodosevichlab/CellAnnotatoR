...
unifyGeneIds <- function(cm, data.gene.id.type, marker.gene.id.type, db=NULL, verbose=FALSE) {
  if (marker.gene.id.type == data.gene.id.type)
    return(list(cm=cm, gene.table=data.frame(data=rownames(cm), marker=rownames(cm), stringsAsFactors=FALSE)))
  ...
  gene.table <- data.frame(data=names(new.ids), marker=as.vector(new.ids), stringsAsFactors=FALSE)
...
parseBlock <- function(block) {
  ...
  for (l in block[2:length(block)]) {
    l.parts <- strsplit(l, ":", fixed=TRUE)[[1]] %>% trimws()
    if (length(l.parts) != 2) {
    ...
    if (l.parts[[1]] == "expressed") {
      genes <- strsplit(l.parts[[2]], ",", fixed=TRUE)[[1]] %>% trimws()
      if (length(genes) == 0)
...
#'
#' @param cm gene count matrix with cells by columns and genes by rows. May be in raw, TC-normalized or tf-idf-normalized format (in case of tf-idf, `prenormalized` must be set to `TRUE`)
...
#' @export
getClassificationData <- function(cm, markers, prenormalized=FALSE, data.gene.id.type="SYMBOL", marker.gene.id.type="SYMBOL", db=NULL, verbose=FALSE) {
  if (!prenormalized) {
...
#' @export
markerListToMarkup <- function(marker.list, file="", group.by.parent=TRUE) {
  if (!group.by.parent) {
    ...
    markup.text <- names(ml.per.parent) %>% lapply(function(pn)
      paste0("## ", pn, "\n\n", markerListToMarkup(ml.per.parent[[pn]], group.by.parent=FALSE))) %>%
      paste0(collapse="")
  }
